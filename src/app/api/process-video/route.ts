...
async function tryDownloadSubs(url: string, tempDir: string): Promise<{
  ...
  for (const lang of langAttempts) {
    try {
      ...
      const subFile = fs.readdirSync(tempDir).find(
...
function generateSegmentFallback(duration: number): {
  ...
  if (duration <= 60) {
    return {
      clips: [
        { start: 0, end: duration, description: "Bagian awal video" },
      ],
    };
  }
  ...
  return {
    clips: [
      {
        start: Math.max(0, quarter - 10),
        end: Math.min(quarter + 20, duration),
        description: "Bagian seperempat pertama video",
      },
      {
        start: Math.max(0, mid + quarter - 10),
        end: Math.min(mid + quarter + 20, duration),
        description: "Bagian akhir video",
      },
    ],
  };
}
...
async function analyzeVideo(context: {
  ...
  if (!hasTranscript && !hasChapters) {
    ...
    return {
      clips: fallback.clips.map((c) => ({
        ...c,
        end: Math.min(c.end, context.duration),
      })),
  ...
  return new Promise((resolve, reject) => {
    ...
    if (proc.stdout) {
      proc.stdout.on("data", (chunk: Buffer) => {
        ...
        if (text.includes("%")) console.log(`yt-dlp: ${text.trim()}`);
      });
    }
  });
}
...
export async function POST(request: NextRequest) {
  const { projectId, youtubeUrl, title: providedTitle, model, provider, accessToken } = await request.json();
  if (!projectId || !youtubeUrl) {
  ...
  if (!ffmpegPath) {
    return NextResponse.json({ error: "FFmpeg binary not found. Run on a machine with ffmpeg installed." }, { status: 500 });
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const context = await getVideoContext(youtubeUrl, tempDir);
        const title = providedTitle || context.title;
        ...
        const selectedModel = selectedProvider === "gemini"
          ? (GEMINI_MODELS.includes(model) ? model : "gemini-2.0-flash")
          : (GROQ_MODELS.includes(model) ? model : "llama-3.1-8b-instant");
        const analysis = await analyzeVideo({ ...context, title, model: selectedModel, provider: selectedProvider });
        send({ progress: 30, step: "Mengunduh video dari YouTube..." });

        const videoPath = await downloadYouTube(
          tempDir, "video.mp4", youtubeUrl,
          (dlPct) => {
        ...
        for (let i = 0; i < analysis.clips.length; i++) {
          ...
          await cutClip(ffmpegPath, tempDir, "video.mp4", clipName, c.start, dur);
          ...
          try {
            ...
            if (convexUrl) {
              await fetch(`${convexUrl}/api/mutation`, {
                ...
                body: JSON.stringify({
                  path: "projects:updateProjectResult",
                  args: {
                    projectId,
                    clips: analysis.clips,
                    captions: analysis.captions,
        ...
        send({ progress: 100, step: "Selesai!", result });
        controller.close();
      } catch (error) {
